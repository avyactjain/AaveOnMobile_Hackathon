...
import 'package:AaveOnMobile/app/aave/common_widgets/borrow_broadcast_success_modal.dart';
...
class BorrowReviewModal extends StatefulWidget {
  ...
  @override
  _BorrowReviewModalState createState() => _BorrowReviewModalState();
}

class _BorrowReviewModalState extends State<BorrowReviewModal> {
        ...
        switch (snapshot.connectionState) {
          ...
          case ConnectionState.done:
            if (snapshot.data.statusCode == 200 &&
              ...
              if (_operation.toLowerCase() == "borrow".toLowerCase()) {
                var borrowApiResponseObj = jsonDecode(snapshot.data.body);
                widget.localTxObj["borrowApiResponseObj"] =
                    borrowApiResponseObj;

                print(borrowApiResponseObj);

                if (borrowApiResponseObj["isError"] != true) {
                  String amountString =
                      widget.localTxObj["txnParamsByUser"]["amount"];

                  String token =
                      widget.localTxObj["borrowApiResponseObj"]["token"];

                  String rawSvgString = Jdenticon.toSvg(widget
                      .localTxObj["borrowApiResponseObj"]["to_addr"]
                      .toString());
                        ...
                        children: <Widget>[
                          ...
                          FutureBuilder(
                            future: GasUtils().getGasPriceInEthAndUsd(
                                borrowApiResponseObj["rawTx"]["gasLimit"],
                                borrowApiResponseObj["rawTx"]["gasPrice"]),
                            builder: (context, snapshot) {
                              switch (snapshot.connectionState) {
                                case ConnectionState.none:
                                  return Container();
                ...
                if (broadcastResponseData["isError"] == false) {
                  return BorrowBroadcastSuccessModal(
                    onDonePressed: () {
                      widget.onDonePressed();
                    },
