...
class SwitchApyReviewModal extends StatefulWidget {
  final Map localTxObj;
  final Future switchApyApiResponse;
  final Function() onDonePressed;

  SwitchApyReviewModal({
    @required this.localTxObj,
    @required this.onDonePressed,
    @required this.switchApyApiResponse,
  });

  @override
  _SwitchApyReviewModalState createState() => _SwitchApyReviewModalState();
}

class _SwitchApyReviewModalState extends State<SwitchApyReviewModal> {
  ...
  void initState() {
    ...
    _futureToBuild = widget.switchApyApiResponse;
    super.initState();
  }
        ...
        switch (snapshot.connectionState) {
          ...
          case ConnectionState.done:
            if (snapshot.data.statusCode == 200 &&
              ...
              if (_operation.toLowerCase() == "SWITCHINTEREST".toLowerCase()) {
                var switchApyApiResponseObj = jsonDecode(snapshot.data.body);
                widget.localTxObj["switchApyApiResponseObj"] =
                    switchApyApiResponseObj;

                print(switchApyApiResponseObj);

                if (switchApyApiResponseObj["isError"] != true) {
                  String _intitialApyType = widget.localTxObj["txnParamsByUser"]
                          ["currentApyType"]
                      .toString()
                      .split(".")
                      .last;
                  // String _finalApyType = widget.localTxObj["txnParamsByUser"]
                  //         ["finalApytype"]
                  //     .toString()
                  //     .split(".")
                  //     .last;

                  String _finalApyType = _intitialApyType.toLowerCase() == "variable" ? "Stable" : "Variable";

                  String token =
                      widget.localTxObj["switchApyApiResponseObj"]["token"];

                  String rawSvgString = Jdenticon.toSvg(widget
                      .localTxObj["switchApyApiResponseObj"]["to_addr"]
                      .toString());
                        ...
                        children: <Widget>[
                          ...
                          SizedBox(
                            child: Column(
                              ...
                              children: <Widget>[
                                Text(
                                  "Switch borrow interest type from $_intitialApyType to $_finalApyType",
                                  style: TextStyle(
                          ...
                          FutureBuilder(
                            future: GasUtils().getGasPriceInEthAndUsd(
                                switchApyApiResponseObj["rawTx"]["gasLimit"],
                                switchApyApiResponseObj["rawTx"]["gasPrice"]),
                            builder: (context, snapshot) {
                              switch (snapshot.connectionState) {
                                case ConnectionState.none:
                                  return Container();
